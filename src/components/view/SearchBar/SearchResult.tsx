...
const { resultList } = style()

interface SearchResultProps extends ComponentPropsWithoutRef<"div"> {
    data: MediaListProps | PersonListProps
    isError: boolean
    isLoading: boolean
}

const SearchResult = ({ data, isError, isLoading, ...rest }: SearchResultProps) => {
    if (isError)
        return (
            <div {...rest}>
    ...
    return (
        ...
        data.results.length > 0 && (
            <div {...rest}>
                <div className="p-2 flex flex-col">{data.results.map(searchResultList)}</div>
            </div>
        )
    )
}
...
function searchResultList({ media_type, ...rest }: MediaProps | PersonProps) {
    if (media_type === "person") {
        ...
        return (
            <Link key={id} href="/">
                <Card className={resultList()} layout="landscape">
                    <CardMedia
                        alt=""
